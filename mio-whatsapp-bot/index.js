...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') console.log('\n✅ BOT ASSISTENZA (SOLO TESTO) ATTIVO!');
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        if (!sessioni[from]) {
            if (!isAperto()) {
                return await sock.sendMessage(from, { text: "Ci dispiace ma siamo chiusi, ci ricontatti domani, questi sono i nostri orari:\nLun: 14:30-20:00\nMar: 16:40-18:00 / 20:00-20:35\nMer: 14:30-20:00\nGio: 16:35-18:00 / 20:00-20:40\nVen: 14:20-21:30\nSab: 07:00-13:30 / 16:00-19:30\nDom: 08:00-12:00" });
            }
            // MESSAGGIO DI BENVENUTO SENZA IMMAGINE
            await sock.sendMessage(from, { text: 'Benvenuto al servizio autonomo di risposta di Edoardo! Ti rispondo io perchè Edoardo Non è disponibile!\nAttendi un secondo che apro un ticket per te!' });
            await delay(10000);
        ...
        if (sessioni[from].stato === 'menu_principale') {
            ...
            await sock.sendMessage(MIO_NUMERO, { text: '📅 *NUOVA PRENOTAZIONE*\nUtente: ' + from + '\nInfo: ' + body });
            ...
            await sock.sendMessage(MIO_NUMERO, { text: '👤 *RICHIESTA OPERATORE*\nNome: ' + body + '\nContatto: ' + from });
            avviaAttesaOperatore(from, sock);
        }
    });
